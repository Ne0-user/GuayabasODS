...
ruta_excel = os.path.join(os.path.dirname(__file__), "8.5.1d_sh_es.xlsx")
...
cursor.execute("""
CREATE TABLE IF NOT EXISTS IHRN (
...
for col in range(col_inicio, ws.max_column + 1):
    ...
    pais_nacimiento = ws.cell(row=4, column=col).value
    cambio_residencia = ws.cell(row=5, column=col).value

    for fila in range(fila_inicio, ws.max_row + 1):

        estado = ws.cell(row=fila, column=2).value  # solo control
        ...
        print(f"{año}, {sexo}, {pais_nacimiento}, {cambio_residencia}, {valor}")

        cursor.execute("""
            INSERT INTO IHRN (año, sexo, pais_nacimiento, cambio_residencia, cantidad)
            VALUES (?, ?, ?, ?, ?)
        """, (año, sexo, pais_nacimiento, cambio_residencia, valor))
