...
ruta_excel = os.path.join(os.path.dirname(__file__), "8.5.1c_sh_es.xlsx")
...
cursor.execute("""
CREATE TABLE IF NOT EXISTS IHOcupacion (
...
for col in range(col_inicio, ws.max_column + 1):
    ...
    ocupacion = ws.cell(row=4, column=col).value
    ...
    for fila in range(fila_inicio, ws.max_row + 1):
        ...
        print(f"{año}, {sexo}, {ocupacion}, {valor}")

        cursor.execute("""
            INSERT INTO IHOcupacion (año, sexo, ocupacion, cantidad)
            VALUES (?, ?, ?, ?)
        """, (año, sexo, ocupacion, valor))
