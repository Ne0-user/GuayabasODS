...
ruta_excel = os.path.join(os.path.dirname(__file__), "8.5.1a_sh_99_es.xlsx")
...
cursor.execute("""
CREATE TABLE IF NOT EXISTS IHEE (
...
for col in range(col_inicio, ws.max_column + 1):
    ...
    for fila in range(fila_inicio, ws.max_row + 1):
        ...
        print(f"{año}, {sexo}, {estado}, {valor}")

        cursor.execute("""
            INSERT INTO IHEE (año, sexo, estado, cantidad)
            VALUES (?, ?, ?, ?)
        """, (año, sexo, estado, valor))
