...
ruta_excel = os.path.join(os.path.dirname(__file__), "8.7.1_sh_es.xlsx")
...
cursor.execute("""
CREATE TABLE IF NOT EXISTS OINP517 (
...
for col in range(col_inicio, ws.max_column + 1):
    ...
    edad = ws.cell(row=4, column=col).value
    ...
    for fila in range(fila_inicio, ws.max_row + 1):
        ...
        print(f"{año}, {genero}, {edad}, {estado}, {valor}")

        cursor.execute("""
            INSERT INTO OINP517 (año, genero, edad, estado, cantidad)
            VALUES (?, ?, ?, ?, ?)
        """, (año, genero, edad, estado, valor))
