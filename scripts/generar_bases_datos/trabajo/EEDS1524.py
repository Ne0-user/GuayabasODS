...
ruta_excel = os.path.join(os.path.dirname(__file__), "8r.6.1_sh_es.xlsx")
...
cursor.execute("""
CREATE TABLE IF NOT EXISTS EEDS1524 (
...
for col in range(col_inicio, ws.max_column + 1):
    ...
    rango_edad = ws.cell(row=4, column=col).value
    ...
    for fila in range(fila_inicio, ws.max_row + 1):
        ...
        print(f"{año}, {sexo}, {rango_edad}, {valor}")

        cursor.execute("""
            INSERT INTO EEDS1524 (año, sexo, rango_edad, cantidad)
            VALUES (?, ?, ?, ?)
        """, (año, sexo, rango_edad, valor))
