...
ruta_excel = os.path.join(os.path.dirname(__file__), "8.5.1b_sh_es.xlsx")
...
cursor.execute("""
CREATE TABLE IF NOT EXISTS IHEdad (
...
for col in range(col_inicio, ws.max_column + 1):
    ...
    for fila in range(fila_inicio, ws.max_row + 1):
        ...
        print(f"{año}, {sexo}, {grupo_edad}, {valor}")

        cursor.execute("""
            INSERT INTO IHEdad (año, sexo, grupo_edad, cantidad)
            VALUES (?, ?, ?, ?)
        """, (año, sexo, grupo_edad, valor))
