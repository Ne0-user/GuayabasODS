...
ruta_excel = os.path.join(os.path.dirname(__file__), "8.2.1_sh_es.xlsx")
...
cursor.execute("""
CREATE TABLE IF NOT EXISTS CPIBP (
...
for col in range(col_inicio, ws.max_column + 1):
    ...
    tasa_crecimiento = ws.cell(row=3, column=col).value
    ...
    for fila in range(fila_inicio, ws.max_row + 1):
        ...
        print(f"{año}, {tasa_crecimiento}, {valor}")

        cursor.execute("""
            INSERT INTO CPIBP (año, tasa_crecimiento, cantidad)
            VALUES (?, ?, ?)
        """, (año, tasa_crecimiento, valor))
